...
public class MoiveImportServlet extends HttpServlet{
	@Override
...
//		if (count!=0&&Global.mdlist.size()!=0){
		...
		Queue queue = QueueFactory.getQueue("MoiveQueue");
			queue.add(TaskOptions.Builder.withUrl("/tasks/import").method(Method.POST));
		}
			//		}
			
		resp.getWriter().print(Global.status);
	}
